...
const TSHIP_SUBTYPES = [
  {
    ...
    tooltip: "Only covers goods staying inside one FTZ \u2014 these lenses need to leave Singapore entirely.",
  },
  {
...
export function BeatB052() {
  ...
  const computeScoreBreakdown = (): {
    ...
    const confirmation = gate3Choice === "OUT" ? 20 : 0;
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      <header className="bg-customs-navy border-b border-customs-border px-3 sm:px-6 py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm sm:text-base font-bold text-white">
            CASE #052 &mdash; PRECISION OPTICS EXPORT
          </h1>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-auto">
        {/* Journey Map — REVERSED for export: Singapore → FTZ → Loaded → Germany */}
        ...
        <div className="flex-1 flex flex-col p-3 sm:p-6 max-w-3xl mx-auto w-full">
          {state === "INTRO" && (
            <div className="flex-1 flex items-center justify-center">
              <div className="bg-customs-panel border border-customs-gold/30 rounded-xl p-6 sm:p-8 text-center max-w-md animate-fade-in">
                <p className="text-3xl mb-3">{"\u{1F52C}"}</p>
                ...
                <p className="text-customs-muted text-sm mb-2">
                  A shipment of 300 precision optical lenses is being exported
                  from Singapore to Germany. Your job: route it through
                  Singapore&apos;s trade checkpoints by choosing the correct
                  permits at each gate.
                </p>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="bg-customs-panel border border-customs-green/30 rounded-xl p-6 sm:p-8 text-center max-w-md animate-fade-in">
                <p className="text-customs-green text-lg font-medium mb-2">
                  {"\u{1F69B}"} Goods arriving at Keppel FTZ for loading...
                </p>
              ...
              <div className="bg-customs-panel border border-customs-green/30 rounded-xl p-6 sm:p-8 text-center max-w-md animate-fade-in">
                ...
                {scaffolding === 1 && (
                  <p className="text-customs-muted text-sm mt-3 mb-4">
                    Moving goods from the factory to the port FTZ is a domestic
                    transfer within Singapore. Now the goods need an export
                    permit to leave.
                  </p>
                )}
                <button
          ...
          {(state === "GATE2_ACTIVE" || state === "GATE2_WRONG") && (
            <div className="animate-fade-in space-y-4" ref={gate2Ref}>
              <div className="bg-customs-panel border border-customs-border rounded-xl p-4 sm:p-6">
                ...
                <p className="text-customs-muted text-sm mb-2">
                  Your lenses need to leave Singapore for Germany. Choose the
                  right permit type to load them onto the vessel.
                </p>
                    ...
                    {!selectedMsg ? (
                      ...
                      <div className="p-3 rounded border border-customs-green/30 bg-customs-green/5">
                        <p className="text-sm text-customs-green">
                          Export permit (standard) &mdash; confirmed
                        </p>
                      </div>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="bg-customs-panel border border-customs-green/30 rounded-xl p-6 sm:p-8 text-center max-w-md animate-fade-in">
                ...
                <p className="text-customs-green text-lg font-medium">
                  Gate 2 unlocked! OUT / Export is correct.
                </p>
                {attempts === 0 && (
                ...
                <p className="text-customs-muted text-sm mt-2">
                  {state === "LEG2_ANIMATING"
                    ? "\u{1F6A2} Goods being loaded onto vessel..."
                    : "Proceeding..."}
                </p>
              </div>
            </div>
            ...
            <div className="animate-fade-in space-y-4">
              <div className="bg-customs-panel border border-customs-border rounded-xl p-4 sm:p-6">
                <h2 className="text-white font-bold text-base sm:text-lg mb-1">
                  LEG 3: LOADED &rarr; DELIVERED
                </h2>
                <p className="text-customs-muted text-sm mb-4">
                  Permit approved. The goods are loaded. Which direction did
                  your permit point &mdash; goods leaving Singapore or entering?
                </p>
                <div className="flex gap-3">
                  {(["OUT", "IN"] as const).map((opt) => (
                    <button
                      key={opt}
                    ...
                    >
                      {opt === "OUT"
                        ...
                        : "Entering Singapore (Import)"}
                    </button>
                  ))}
                </div>
                {gate3Choice === "IN" && (
                  <p className="text-customs-amber text-sm mt-3 animate-fade-in">
                    {"\u26A0\uFE0F"} The lenses are leaving Singapore, not
                    coming in. This was an export permit. Select the correct
                    direction above.
                  </p>
                )}
                <button
                  onClick={handleGate3}
                  disabled={gate3Choice !== "OUT"}
                ...
                >
                  {gate3Choice === "IN"
                    ? "SELECT \u201CLEAVING SINGAPORE\u201D ABOVE"
                    : gate3Choice === "OUT"
                      ? "CONFIRM"
                      : "SELECT A DIRECTION"}
                </button>
          ...
          {state === "GATE3_CORRECT" && (
            <div className="flex-1 flex items-center justify-center">
              <div className="bg-customs-panel border border-customs-green/30 rounded-xl p-6 sm:p-8 text-center max-w-md animate-fade-in">
                <p className="text-4xl mb-3">{"\u{1F513}"}</p>
                <p className="text-customs-green text-lg font-medium">
                  Confirmed! Shipping to Germany...
                </p>
                <p className="text-customs-muted text-sm mt-2">
                  {"\u{1F6A2}"} Vessel departing for Hamburg...
                </p>
              </div>
            </div>
          )}
          ...
          {state === "LEG3_ANIMATING" && (
            <div className="flex-1 flex items-center justify-center">
              <div className="bg-customs-panel border border-customs-green/30 rounded-xl p-6 sm:p-8 text-center max-w-md animate-fade-in">
                <p className="text-4xl mb-3">{"\u{1F6A2}"}</p>
                <p className="text-customs-green text-lg font-medium">
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="bg-customs-panel border border-customs-gold/30 rounded-xl p-6 sm:p-8 text-center max-w-md animate-fade-in">
                ...
                <h2 className="text-customs-gold text-xl font-bold mb-4">
                  SHIPMENT DELIVERED &mdash; CASE #052
                </h2>
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-customs-muted">
                      LEG 1: Singapore &rarr; Port FTZ
                    </span>
        ...
        <div className="border-t border-customs-border bg-customs-navy flex-shrink-0">
          ...
          {caseCardOpen && (
            <div className="px-3 sm:px-6 pb-3 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs animate-fade-in">
              <div>
                <span className="text-customs-muted">Exporter</span>
                <p className="text-white">NanoOptics Pte Ltd, SG</p>
              </div>
              <div>
                <span className="text-customs-muted">Buyer</span>
                <p className="text-white">Carl Zeiss AG, DE</p>
              </div>
              <div>
                <span className="text-customs-muted">Goods</span>
                <p className="text-white">
                  Precision optical lenses (300 U)
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-white">Hamburg, Germany</p>
              </div>
              <div>
                <span className="text-customs-muted">INCOTERM</span>
                <p className="text-white">FOB Singapore</p>
              </div>
              <div>
                <span className="text-customs-muted">Status</span>
                <p className="text-customs-green">Non-dutiable</p>
              </div>
            </div>
          )}
        </div>
            ...
            <div className="space-y-4 text-sm">
                ...
                <ul className="text-customs-muted space-y-2 list-none p-0">
                  <li className="border-b border-customs-border/30 pb-2">
                    ...
                    <span className="text-xs">
                      Goods arriving in Singapore for local consumption or use.
                    </span>
                  </li>
                  <li className="border-b border-customs-border/30 pb-2">
                    ...
                    <br />
                    <span className="text-xs">
                      Goods leaving Singapore to an overseas destination. Requires an export permit.
                    </span>
                  </li>
                  <li className="border-b border-customs-border/30 pb-2">
                    <strong className="text-white">Transhipment/Movement</strong>
                    <br />
                    <span className="text-xs">
                  ...
                  <li>
                    <strong className="text-white">COO (Certificate of Origin)</strong>
                    <br />
                    <span className="text-xs">
                      A trade document proving where goods were manufactured. Not a movement permit.
                    </span>
                  </li>
                </ul>
              </div>
              <div className="bg-customs-surface/50 rounded-lg p-3 border border-customs-border/50">
                ...
                <p className="text-customs-muted text-xs">
                  Moving goods from a Singapore factory to a port FTZ is a
                  domestic transfer. But once goods leave the FTZ to be loaded
                  onto a vessel heading overseas, an export (OUT) permit is required.
                </p>
              </div>
