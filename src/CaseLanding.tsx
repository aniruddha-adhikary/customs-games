...
const BEATS = [
  ...
  },
  {
    ...
    duration: "~12 min",
    description:
      "Active permits on the board. A ticking clock. Triage correctly.",
    icon: "\u23F1\uFE0F",
...
export function CaseLanding({ caseId }: { caseId: string }) {
  const caseConfig = CASES[caseId];
  if (!caseConfig) return null;

  const basePath = `/case-${caseId}`;
  const beatIcon = caseId === "025" ? "\u2708\uFE0F" : "\u{1F6A2}";
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8">
      <header className="text-center mb-8 sm:mb-12">
        <Link
          to="/"
          className="text-customs-muted hover:text-white text-sm no-underline mb-4 inline-block"
        >
          &larr; All Cases
        </Link>
        <div className="text-4xl mb-3">{caseConfig.icon}</div>
        <h1 className="text-3xl sm:text-5xl font-bold text-white mb-2">
          {caseConfig.title}
        </h1>
        <p className="text-customs-gold text-base sm:text-lg">
          {caseConfig.subtitle}
        </p>
        ...
        {BEATS.map((beat) => (
          ...
          >
            ...
            <div className="text-3xl sm:text-4xl mb-3">
              {beat.id === "beat-b" ? beatIcon : beat.icon}
            </div>
